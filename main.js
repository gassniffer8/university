...
function draw() {
    ...
    let translate = torchCylind(map(userPoint.x, 0, 1, 0, 1), map(userPoint.y, 0, 1, -1, 1))
...
function CreateSurfaceData() {
    ...
    const MAX = 1
    const STEP = 0.005
    for (let uc = 0; uc < MAX; uc += STEP) {
        for (let vc = -1; vc < MAX; vc += STEP) {
            const v1 = torchCylind(uc, vc);
            const v2 = torchCylind(uc + STEP, vc);
            const v3 = torchCylind(uc, vc + STEP);
            const v4 = torchCylind(uc + STEP, vc + STEP);
...
function CreateSurfaceTextureData() {
    ...
    for (let uc = 0; uc < MAX; uc += STEP) {
        for (let vc = -1; vc < MAX; vc += STEP) {
            let u = map(uc, 0, MAX, 0, 1)
            let v = map(vc, -1, MAX, 0, 1)
            vertexTextureList.push(u, v)
            u = map(uc + STEP, 0, MAX, 0, 1)
            vertexTextureList.push(u, v)
            u = map(uc, 0, MAX, 0, 1)
            v = map(vc + STEP, -1, MAX, 0, 1)
            vertexTextureList.push(u, v)
            u = map(uc + STEP, 0, MAX, 0, 1)
            v = map(vc, -1, MAX, 0, 1)
            vertexTextureList.push(u, v)
...
function torchCylind(u, v) {
    const H = 1;
    const c = 2;
    const a = 0.1 * Math.PI;
    const fi = 0;
    let x = c * u + v * (Math.sin(fi) + Math.tan(a) * Math.cos(fi) * Math.cos(t(u)));
    let y = v * Math.tan(a) * Math.sin(t(u));
    let z = H + v * (Math.tan(a) * Math.sin(fi) * Math.cos(t(u)) - Math.cos(fi));
    const COEF = 1;
    return {
        x: x * COEF,
        y: y * COEF,
